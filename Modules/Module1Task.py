...
class thread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            while True:
                GPIO.output(self.led_pin_1, GPIO.HIGH) # Turn LED1 on
                time.sleep(self.delay) # Delay for 1 second
                GPIO.output(self.led_pin_1, GPIO.LOW) # Turn LED1 off
                time.sleep(self.delay) # Delay for 1 second
                GPIO.output(13, GPIO.HIGH) # Turn LED2 on
                time.sleep(self.delay) # Delay for 1 second
                GPIO.output(13, GPIO.LOW) # Turn LED2 off
                time.sleep(self.delay) # Delay for 1 second
                GPIO.output(self.led_pin_3, GPIO.HIGH) # Turn LED3 on
                time.sleep(self.delay) # Delay for 1 second
                GPIO.output(self.led_pin_3, GPIO.LOW) # Turn LED3 off
                time.sleep(self.delay) # Delay for 1 second
